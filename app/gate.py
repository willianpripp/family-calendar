...
# to mint a hash for CAL_GATE_USERS. Unconfigured means fail CLOSED for public
...
def _secret() -> bytes:
    return os.environ.get("CAL_GATE_SECRET", "").strip().encode()


def _users() -> dict[str, str]:
    """CAL_GATE_USERS="willian=pbkdf2:600000:<salt>:<hash>,aline=..." """
    out = {}
    for entry in os.environ.get("CAL_GATE_USERS", "").split(","):
        entry = entry.strip()
        if "=" not in entry:
            continue
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("usage: python gate.py 'the-password'  -> a hash for CAL_GATE_USERS")
        raise SystemExit(2)
